...
router.put(function (req, res) {
  Stat.findById(req.params.number_of_views, function (err, nbViews) {
    if (err) {
      res.send(err);
    }
    // Modifie l'état de changement des nombres de vues
    nbViews.status = req.body.status;
    nbViews.save(function (err) {
      if (err) {
        res.send(err);
      }
      res.json({ message: "Evolution nombres de vues initialisé" });
    });
  });
});
